...
export default class Confetti extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image('containerNarrow', 'assets/6bf07bd9730fd6f942b90df74d7987ae1ed2bb5c.png');
    // Buttons
  ...
  create() {
    ...
    const addToContainer = (key, figmaX, figmaY, isVisible = true) => {
      ...
      // X_phaser = X_figma - halfW + (Width_asset / 2)
      img.setPosition(
  ...
  createControls() {
    // --- UI PANEL (Tweakpane) ---
    const { pane, folder } = createPane(this, 'Confetti', 'Confetti');
    this.pane = pane;
    folder.addButton({ title: 'Fire Confetti' }).on('click', () => {
      this.fireConfetti();
    });
  }
}
